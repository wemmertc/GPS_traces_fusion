...
parser.add_option("-m", "--tmatch", dest="tmatch", default=50, help="Give your maximal value for intersection matching. Default = 50m")

(options, args) = parser.parse_args()

if not options.fileloc: parser.error('Missing result file')
if not options.truthloc : parser.error('Missing ground truth file')
...
# F-score calculation
mappedTruthIntersections = [key for key in mapping.keys()]

TP = [value[0] for value in mapping.values()]
FP = [i for i in intersections if i not in TP]
FN = [it for it in intersectionsTruth if (it[0], it[1]) not in mappedTruthIntersections]

precision = numpy.around(len(TP) / (len(TP)+len(FP)), 2)
recall = numpy.around(len(TP) / (len(TP) + len(FN)), 2)
...
print("### Intersections evaluation ###")
print("Acceptation distance = " + str(Tmatch) + "m")
...
for intersection in TP :
    features.append(Feature(geometry=Point(intersection), properties={'marker-color': '#3fc700', 'title':'True postive'}))

for intersection in FP :
    features.append(Feature(geometry=Point(intersection), properties={'marker-color': '#d11f1f', 'marker-symbol':'roadblock', 'title':'False positive'}))

for intersection in FN :
    features.append(Feature(geometry=Point(intersection), properties={'marker-color': '#545454', 'marker-symbol':'cross', 'title':'False negative'}))
...
precision = numpy.around(len(TN) / len(segments), 2)
recall = numpy.around(len(TN) / len(segmentsTruth), 2)
fscore = numpy.around((2 * recall * precision) / (recall + precision), 2)
